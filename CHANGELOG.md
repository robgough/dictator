...
- Dictation now has named Modes that bundle the LLM passes together. Two ship by default: Quick (no LLM polish — Whisper's raw transcript ships straight through, fastest path) and Write (all three passes — your old setup, with your existing tuned prompts preserved). Tap Tab while holding the dictation hotkey to cycle between modes mid-recording — the active mode and the next one in the cycle are shown in the HUD. Pick the default mode from the menu bar. Add or customise modes in Settings → Modes: each mode has its own pre-processing toggles (spoken cues, vocabulary application), Pass 1 / 2 / 3 toggles, prompt addenda, and an "Auto-activate for apps" list so a mode can switch on automatically when you start dictating into a specific app (e.g. Mail.app). Duplicate Quick and turn off spoken cues + vocabulary to get a fully raw transcription mode.
- Saying "question mark", "exclamation mark", or "full stop" no longer leaves a stray period or comma in the output. Whisper itself routinely transcribes these as "Question mark." (with a trailing full stop); the substitution pass now collapses the redundant soft punctuation that used to leak through.
