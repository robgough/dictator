...
- A bigger set of spoken-punctuation cues. New single-word: "hyphen" (-), "em dash" (—), "en dash" (–), "tilde" (~), "asterisk" (*), "ampersand" (&), "underscore" (_), "backtick" (`), "caret" (^), "ellipsis" (…), "backslash" (\). New multi-word: "at sign" (@), "hash sign" / "pound sign" (#), "dollar sign" ($), "forward slash" (/), "open bracket" / "close bracket" ([]), "open brace" / "close brace" ({}). Arithmetic operators kick in only when surrounded by numbers, so normal English isn't disturbed: "5 plus 3" → "5 + 3", "5 percent" → "5%", "5 slash 3" → "5/3", and the same for "minus", "times", "divided by", "equals". Word-form numbers are recognised too, up to the hundreds of thousands — "five percent" → "5%", "twenty-five plus three" → "25 + 3", "one hundred and three percent" → "103%", "twenty thousand and sixty two slash four" → "20062/4". The optional "and" works either way ("one hundred three" or "one hundred and three"). Only fires when the word-number sits next to an operator word — "five apples" stays as written.
