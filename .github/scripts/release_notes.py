...
Three subcommands, each tiny enough that the release workflow stays readable:
    ...
    extract                 Print the body of '## Unreleased' to stdout.
    html                    Read markdown bullets on stdin, write HTML to stdout.
    version <VER> <DATE>    Rewrite CHANGELOG.md in place: move 'Unreleased' to
...
def version_unreleased(version: str, date: str) -> None:
    """Rewrite CHANGELOG.md in place: move Unreleased body under a versioned
...
def main() -> int:
    ...
    if cmd == "version":
        if len(sys.argv) != 4:
            print("usage: release_notes.py version <VERSION> <DATE>", file=sys.stderr)
            return 1
        version_unreleased(sys.argv[2], sys.argv[3])
        return 0
    print(__doc__, file=sys.stderr)
    return 1
