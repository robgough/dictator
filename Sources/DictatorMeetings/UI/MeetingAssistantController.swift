...
///   - the Assistant ▸ Ask About These Notes menu command (⌘⌥A), which
///     toggles listening on and off (`toggleFromCommand`).
///
/// Owned by the notes view for the lifetime it's on screen, and registered with
/// `MeetingsAppState.meetingAssistant` so the menu command can find it.
@MainActor
@Observable
final class MeetingAssistantController {
    ...
    var canRun: Bool {
        session?.meta.notes != nil && ProviderRegistry.shared.provider(for: .final) != nil
    }
    ...
    /// The Assistant ▸ Ask About These Notes menu command (⌘⌥A). A menu
    /// command is a single fire, not a press-and-hold like the hotkey it
    /// replaces, so it toggles: first press opens the dialog and starts
    /// listening, second press stops and runs. Anything already running is
    /// left alone rather than interrupted.
    func toggleFromCommand() {
        if isListening {
            endListeningAndRun()
        } else if !isRunning, !isTranscribing {
            beginListening()
        }
    }
    ...
    private func stopListening(thenRun: Bool) {
        ...
        isTranscribing = true
        // Parakeet only: Dictator Meetings never ships the Whisper path (the
        // meetings recorder is Parakeet end to end), so there's no engine
        // choice to honour here.
    ...
    func run() {
        ...
        errorText = nil
        result = nil
        let prompt = MeetingsAppState.shared.settings.effectiveAssistantPrompt
