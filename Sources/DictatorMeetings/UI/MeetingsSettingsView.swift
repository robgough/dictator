            ...
            Section {
                ForEach(ModelCatalog.diarizationModels) { model in
                    ModelRow(
                        ...
                        isActive: s.settings.effectiveDiarizationModelID == model.id,
                        isLoaded: DiarizerServiceHolder.shared.currentModelID == model.id,
                        isVerifying: manager.verifyingDiarization.contains(model.id),
                        select: { s.settings.diarizationModelID = model.id; state.save() },
