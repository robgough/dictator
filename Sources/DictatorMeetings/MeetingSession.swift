...
final class MeetingSession: Identifiable {
    ...
    func startRecording(
        ...
        recorder.onReady = { [weak self] in
            ...
            MeetingsAppState.shared.meetingRecordingStartedAt = Date()
            self.startTimerLoop()
            ...
            MeetingsAppState.shared.activeCoachEngine = self.coachEngine
            ...
            if MeetingsAppState.shared.settings.meetingCalendarMatchingEnabled {
        ...
        recorder.onUnexpectedStop = { [weak self] reason in
            ...
            MeetingsAppState.shared.activeCoachEngine = nil
            MeetingsAppState.shared.meetingRecordingStartedAt = nil
        ...
        if MeetingsAppState.shared.settings.meetingLiveTranscriptEnabled {
            let liveModelID = MeetingsAppState.shared.settings.parakeetModelID
        ...
        if MeetingsAppState.shared.settings.meetingCoachEnabled, !coachDisabled {
        ...
        if let live = liveTranscriber {
            ...
            let notesEnabled = MeetingsAppState.shared.settings.meetingLiveNotesEnabled
        ...
        do {
            ...
            if MeetingsAppState.shared.settings.meetingCaptureScreenshots {
                startScreenCapture()
            }
        } catch {
    ...
    func stopRecording(parakeetModelID: String) async {
        ...
        if !liveNotesMarkdown.isEmpty {
            let live = MeetingNotes(
                ...
                modelID: MeetingSummaryService.engineModelID(),
                generatedAt: Date(),
                isFinal: false
            )
    ...
    func runProcessor(parakeetModelID: String) async {
        ...
        do {
            ...
            let settings = MeetingsAppState.shared.settings
            let llmAvailable = ProviderRegistry.shared.provider(for: .final) != nil
            state = .ready
    ...
    ///
    /// The LLM container is deliberately NOT unloaded — the user's next act
    /// may well be "generate the notes" — but its GPU buffer pool is handed
    /// back, which is the part that actually grows. That last step is
    /// conditional: `releaseGPUCache` only means anything when THIS process
    /// ran an MLX model, and in the common configuration it didn't (the
    /// Dictator socket borrows the model from the other process, and cloud
    /// providers hold no GPU state at all). Calling it anyway would be
    ...
    private func maybeAutoRename(settings: MeetingsSettings) async {
    ...
    private func inferSpeakerNames(settings: MeetingsSettings) async {
        guard let transcript = MeetingStorage.readTranscript(for: id) else { return }
    ...
    func generateCoachReport(settings: MeetingsSettings) async {
