...
final class MeetingNotesAccumulator {
    ...
    @ObservationIgnored private let settings: MeetingsSettings
    ...
    func start() {
        ...
        // Live notes go through the `.live` slot — a fast/cheap backend, not
        // necessarily the one that writes the final pass.
        Task { try? await ProviderRegistry.shared.provider(for: .live)?.prepare() }
    ...
    private func runChecklistPass(lines: [MeetingLiveTranscriber.LiveLine]) async {
        guard let coach, let provider = ProviderRegistry.shared.provider(for: .live) else { return }
    ...
    private func runPass(newLines: [MeetingLiveTranscriber.LiveLine]) async {
        guard let provider = ProviderRegistry.shared.provider(for: .live) else { return }
        inflight = true
        isThinking = true
        defer { inflight = false; isThinking = false }
