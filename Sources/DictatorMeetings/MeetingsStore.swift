...
final class MeetingsStore {
    ...
    /// style menu on the Notes tab's Write notes / Re-run control — it
    ...
    private func applyAutoDeleteIfConfigured() {
        let days = MeetingsAppState.shared.settings.meetingAutoDeleteAfterDays
    ...
    private func applyAudioRetentionIfConfigured() {
        let days = MeetingsAppState.shared.settings.meetingAudioRetentionDays
        guard days > 0 else { return }
        guard let cutoff = Calendar.current.date(byAdding: .day, value: -days, to: Date()) else { return }
