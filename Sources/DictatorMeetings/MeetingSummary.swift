...
enum MeetingSummaryService {
    ...
    /// How much transcript we're willing to put into ONE call, derived from
    /// the provider that will actually answer rather than the old flat 6 000.
    /// A 128k cloud model shouldn't be map-reduced into fragments, and a 4k
    /// Apple window mustn't be handed 24 000 characters.
    ///
    /// The window has to hold prompt AND reply, so the reply budget plus a
    /// generous allowance for the system prompt (which carries the type
    /// template, the user's addendum and the one-off instruction — it runs to
    /// a couple of thousand tokens on a customised install) comes off the top.
    /// The floor keeps a tiny window from producing hundreds of chunks; the
    /// ceiling is a quality decision, not a capacity one — past roughly an
    ...
    static func generateNotes(
        ...
        let inputBudgetTokens = singlePassInputBudgetTokens(for: provider)
        ...
        if approxTokens <= inputBudgetTokens {
            ...
            raw = try await runMapReduce(
                ...
                rawOutline: rawOutline,
                pad: pad,
                chunkBudgetTokens: inputBudgetTokens
            )
        }
    ...
    private static func runMapReduce(
        ...
        let chunkBudgetChars = chunkBudgetTokens * 4
        ...
        let finalResult = try await provider.assist(
            selection: merged,
            instruction: instruction,
            systemPrompt: systemPrompt,
            cancellation: { Task.isCancelled }
    ...
    static func detectMeetingType(
        ...
        guard let provider = ProviderRegistry.shared.provider(for: .final) else { return nil }
        let segments = transcript.segments
        ...
        do {
            try await provider.prepare()
            result = try await provider.assist(
                selection: lead,
    ...
    /// What gets stamped into `MeetingNotes.modelID` and shown in the notes
    /// footer. With providers this is the resolved provider's user-facing
    /// label rather than a raw HuggingFace id — "which of my configured
    /// backends wrote this?" is the question the footer answers now, and the
    /// user named those backends themselves.
    ///
    /// Resolved (not configured): if the Dictator slot fell back to the local
    /// model, the notes should say so.
    static func engineModelID(slot: ProviderSlot = .final) -> String {
        ProviderRegistry.shared.provider(for: slot)?.displayName ?? "none"
    }
}
