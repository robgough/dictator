...
public enum MeetingType: String, Codable, CaseIterable, Sendable {
    ...
    public var promptAddendum: String {
        ...
        case .interview:
            ...
            Keep `## Discussion` for the substance that isn't a Company or Candidate fact: how the conversation flowed, themes, the questions asked and how they were answered. After Discussion, add a `## Quotes` section with two or three short VERBATIM lines from the transcript that capture how the person thinks or talks — each bullet the exact words in double quotes, speaker named at the end (e.g. `- "We rebuilt the whole pipeline in a weekend." — Sanah [23:14]`). Never paraphrase inside quotation marks; omit the section if nothing stands out. If something was left unanswered or needs verifying, list it under a `## Open questions` section before Decisions (omit the section when there are none). Decisions usually capture "advance" / "do not advance" / "another round" when the transcript makes that explicit. Action items go to whoever owns the next step exactly as named — interviewer, recruiter, or candidate — and include stated process commitments ("they'll hear back by Friday").
            """
        case .clientCall:
            return """
