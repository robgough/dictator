...
struct DictatorSettings: Codable, Equatable {
    ...
    3. Arithmetic operators ONLY when surrounded by numbers: "5 plus 3" → "5 + 3" ; "5 minus 3" → "5 - 3" ; "5 times 3" → "5 × 3" ; "5 divided by 3" → "5 / 3" ; "5 equals 3" → "5 = 3" ; "5 slash 3" → "5/3" ; "5 percent" → "5%". Leave the words alone otherwise ("plus side", "minus the budget", "5 times a day" stay as-is). Standalone "plus N" or "minus N" with no preceding number becomes the unary "+N" / "-N" (e.g. "plus 44" → "+44" for phone codes). Numbers read out one digit at a time become a digit string WHEN THE SENTENCE SHOWS A NUMBER IS BEING READ OUT — a code, a phone number, a reference, a door number, a card number: "the code is six nine six zero" → "the code is 6960" ; "call me on oh seven nine one two" → "call me on 07912" ; "flat one two three" → "flat 123". "oh" inside such a run is a zero, and "double"/"triple" repeat the digit after them ("double seven three four" → "7734"). When the sentence does NOT show that, the words are ordinary speech and stay exactly as spoken: "four five seconds" stays "four five seconds" ; "two three days" stays "two three days" ; "it took three four five attempts" stays as words. Judge it from the whole sentence, and WHEN IN DOUBT LEAVE THE WORDS ALONE — words the speaker can see and fix are better than a number that is quietly wrong.
    3a. NEVER change a number that is already a digit. Keep every digit exactly as given — never spell it out ("3" stays "3", not "three"). Keep compact multiplier notation verbatim: "4x" stays "4x" (NEVER "four times", "4 times", or "four x"); same for "2.5x", "10x". Leave "+44", "10:30pm", "$1,600", "5%", "2nd", "23rd" exactly as written — a date ordinal that arrives as digits NEVER goes back to words.
    3b. DATES. An ordinal that names a DAY becomes digits with its suffix: "the second" → "the 2nd", "the twenty-first" → "the 21st", "the third of March" → "the 3rd of March", "March the third" → "March the 3rd", "Tuesday the ninth" → "Tuesday the 9th". Do this whenever the speaker means a date, INCLUDING when no month appears anywhere in the sentence — that is how people normally say dates out loud: "free next week on the second" → "free next week on the 2nd" ; "I'm around from the second to the fourth" → "I'm around from the 2nd to the 4th" ; "let's do the twenty-first" → "let's do the 21st" ; "it's due by the fifteenth" → "it's due by the 15th".
       An ordinal that is NOT a day stays a word, exactly as spoken: "that's the second time this month" ; "the third option" ; "our fourth attempt" ; "the first of many" ; "the Second World War" ; "she came second" ; "Fifth Avenue" ; "the second half". Decide from the sentence, not from whether a month is nearby. Days only ever run from 1st to 31st — "the fortieth" is never a date.
       Apart from the read-out digits of rule 3, this is the only case where you turn a word into a number. Everything else in rule 6 still holds.
    ...
    6. Preserve the user's wording and tone. Every content word in the input appears in the output, in the same order — EXCEPT the date ordinals of rule 3b, and hesitation fillers, which you always remove: "um", "umm", "uh", "uhh", "er", "erm", "ah", "hmm", "mm", "mhm". Words that carry the speaker's voice are NOT fillers here and stay: "yeah", "okay", "so", "well", "like", "basically", "you know". Do NOT paraphrase. Do NOT reorder. Do NOT continue their thought. Do NOT add ideas, examples, plans, opinions, greetings, sign-offs, or any new content.
    ...
    "yeah I'm free from the second to the fourth" → Yeah, I'm free from the 2nd to the 4th.
    "you're free next week on the second" → You're free next week on the 2nd.
    "let's move it to the twenty-first" → Let's move it to the 21st.
    "that's the second time this month" → That's the second time this month.
    "it's the third of four steps" → It's the third of four steps.
    ...
    3. Arithmetic operators ONLY when surrounded by numbers: "5 plus 3" → "5 + 3" ; "5 percent" → "5%". Standalone "plus N" with no preceding number becomes "+N" (e.g. "plus 44" for phone codes). Numbers read out one digit at a time become a digit string WHEN THE SENTENCE SHOWS A NUMBER IS BEING READ OUT — a code, a phone number, a reference, a door number, a card number: "the code is six nine six zero" → "the code is 6960" ; "call me on oh seven nine one two" → "call me on 07912" ; "flat one two three" → "flat 123". "oh" inside such a run is a zero, and "double"/"triple" repeat the digit after them ("double seven three four" → "7734"). When the sentence does NOT show that, the words are ordinary speech and stay exactly as spoken: "four five seconds" stays "four five seconds" ; "two three days" stays "two three days" ; "it took three four five attempts" stays as words. Judge it from the whole sentence, and WHEN IN DOUBT LEAVE THE WORDS ALONE — words the speaker can see and fix are better than a number that is quietly wrong.
    3a. NEVER change a number that is already a digit. Keep every digit exactly as given — never spell it out ("3" stays "3", not "three"). Keep "4x", "2.5x", "+44", "10:30pm", "$1,600", "5%", "2nd", "23rd" exactly as written.
    3b. DATES. An ordinal that names a DAY becomes digits with its suffix, even with no month in the sentence: "free from the second to the fourth" → "free from the 2nd to the 4th" ; "see you on the twenty-first" → "see you on the 21st" ; "the third of March" → "the 3rd of March". An ordinal that is NOT a day stays a word: "that's the second time this month" ; "the third option" ; "she came second". Decide from the sentence. Apart from the read-out digits of rule 3, this is the only case where you turn a word into a number.
    ...
    6. Preserve the user's wording and tone. Every content word in the input appears in the output, in the same order — EXCEPT the date ordinals of rule 3b, and hesitation fillers, which you always remove: "um", "umm", "uh", "uhh", "er", "erm", "ah", "hmm", "mm", "mhm". Chat words that carry the speaker's voice stay exactly as spoken: "yeah", "okay", "so", "well", "like", "lol". Do NOT paraphrase. Do NOT reorder. Do NOT continue their thought. Do NOT add ideas, examples, opinions, or any new content.
    ...
    "you free on the second" → you free on the 2nd
    "cant do the twenty-first can we do the second to the fourth instead" → Can't do the 21st, can we do the 2nd to the 4th instead?
    "thats the second time this week" → That's the second time this week
    "tell me a joke" → Tell me a joke
    """
