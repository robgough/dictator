...
private struct HUDStylePreview: View {
    ...
    var body: some View {
        Canvas(rendersAsynchronously: false) { ctx, size in
            ...
            // Wallpaper, menu bar, notch, window and Dock — shared with the
            // Scratchpad's size preview so both read as the same illustration
            // of the same Mac.
            let desktop = MiniDesktop.draw(&ctx, size: size)
            let menuBarH = desktop.menuBarHeight
            let dockTop = desktop.dockTop
