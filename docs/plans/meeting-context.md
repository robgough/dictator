...
2. **v2 — People store**: `people.json`, embedding persistence + matching, `personID` on speakers, manual link/unlink UI, single default-on toggle + per-person delete. — **SHIPPED 2026-06-13** (except manual link/unlink, deferred — auto-linking + rename propagation cover the main flows; revisit if mislinks show up in practice). Implementation notes: unified per-speaker centroids threaded out of `MeetingProcessor.unifySpeakerSpace` → `session.speakerEmbeddings`; link pass runs after `inferSpeakerNames` in `runProcessor`; match threshold 0.74 cosine, same-model embeddings only (k-recent ≤8 per person, model change resets the set); named strangers are learned, anonymous speakers never spawn person records; manual rename propagates to the linked person; calendar attendees donate emails by first-name match. `peopleRecognitionEnabled` default on (synced); PeopleEditor sheet in Meetings settings with per-person delete purging embeddings. Refined same day: the NAME bridges voices the matcher misses (same person, new room/mic) — exact-unique name match links instead of creating a duplicate person, both in the post-pass and on manual rename, and the new environment's embedding is learned under them; ambiguous names never auto-merge. A ≥0.92-similarity novelty guard keeps the per-person sample set diverse (many environments) instead of eight copies of the usual desk mic. Second refinement (2026-06-13): a manual rename of an unlinked speaker now CREATES the person when nobody by that name exists (previously only the post-pass created people, so a hand-typed name after processing left no record — embedding attached when still in memory, embedding-less otherwise and learned via the name bridge next meeting); and ambiguous names (2+ same-named people) now leave the speaker unlinked everywhere instead of the post-pass spawning a third same-named record. Person MERGE shipped same day (was the deferred manual-link bucket's first real need): right-click a row in PeopleEditor → Merge into (confirm-gated, unlike speaker merge, because a wrong person merge poisons recognition globally); `PeopleStore.merge` unions emails + same-model embeddings through the novelty guard, `MeetingsStore.repointPerson` rewrites past metas' personIDs so history follows the survivor; same-named rows get a warning badge. EXCLUSIVE per-meeting assignment (2026-06-13): the old linker matched each speaker independently, so two speakers could both link to one person — over-split (fine) OR two similar voices (the loser's embedding poisons the winner's record, self-reinforcing). Now `linkPeopleAcrossMeetings` is three phases against a `claimedPeople` set: existing links claim first, then voice candidates assigned globally-greedy best-similarity-first (one person per speaker; a speaker whose best is taken falls to next-best via `PeopleStore.matches` returning the full ranked list, not just `bestMatch`), then name-bridge/stranger only onto unclaimed people. `bestMatch` removed. Manual rename still allows two chips → one person deliberately (user asserting an over-split). Editor shows the person count. (Per-person/total voice-data size was added then removed at the user's call — 256-dim float32 × ≤8 ≈ 22 KB/person max, too small to be worth the pixels.) FIXED latent bug: `people.json` was missing from `SyncedStorage.relocateContents` — changing the synced folder stranded the people store; added.
