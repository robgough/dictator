...
3. **Presets + checklist**: `CoachConfig` on `MeetingTypeDefinition`, built-in coach configs, client profiles (`CoachChecklistProfile` + editor), `CoachPresetSheet` in the record flow, checklist watcher in the accumulator loop, checklist in chip + live pane, ad-hoc quick-add + pad lift + `reminder-pending`. — **SHIPPED 2026-06-12.** Implementation notes: coach configs attached to the EXISTING built-ins Interview and Client call (the registry already had them — no near-duplicate "initial client meeting" type added); `askQuestion` nudge added (preset-armed only); checklist outcomes flow stop→`pendingCoachOutcome`→`finaliseCoachMetrics`, with `coach-live.json` (debounced, local) as the crash path; island expanded state grants the panel key-window status only while open (ScratchpadPanel's nonactivating+key pattern). **Revised same day on user feedback: the pre-record `CoachPresetSheet` is GONE** — it slowed recording start, and the user wants zero ceremony. The checklist builds *inside* the meeting instead: the Key points card is always present while the coach runs, with typed/pasted adds (markdown bullets stripped; multi-line pastes land as `preset` so they don't each arm reminders), an "Add from set" menu (built-in type checklists + saved profiles + save-current-as-set), the `!` pad lift, and a "Show on island" button to un-hide the strip. Consequences: `meta.meetingType` is no longer set by the coach (the existing footer type row covers it), `askQuestion` currently has no arming surface (sets don't arm nudges — revisit if wanted), and `meetingLastPresetTypeID`/`meetingLastProfileIDs` settings are dormant.
4. **Report**: `MeetingCoachReport.generateReport`, Coach tab (`CoachReportView`), prompt addendum/override settings. — **SHIPPED 2026-06-13.** Implementation notes: deliberately NOT a transcript map-reduce — the report's grounding is the computed metrics + checklist outcomes + rubric, with the meeting's (already-generated) notes as content context, so it's a single cheap pass riding the notes Generate action. Rubric resolves preset → explicit type → notes-detected type (the retroactive-rubric decision). Coach tab is the fifth transcript tab, shown only when `meta.coach` exists. Deferred: a Settings UI for `meetingCoachPromptAddendum`/`Override` (the keys work via the synced settings.json; add an editor sheet when someone actually wants the warmth lever).
