...
3. **Presets + checklist**: `CoachConfig` on `MeetingTypeDefinition`, built-in coach configs, client profiles (`CoachChecklistProfile` + editor), `CoachPresetSheet` in the record flow, checklist watcher in the accumulator loop, checklist in chip + live pane, ad-hoc quick-add + pad lift + `reminder-pending`. — **SHIPPED 2026-06-12.** Implementation notes: coach configs attached to the EXISTING built-ins Interview and Client call (the registry already had them — no near-duplicate "initial client meeting" type added); `askQuestion` nudge added (preset-armed only); profile creation/deletion lives inline in the preset sheet (a dedicated `MeetingTypeEditor` coach section for custom types is deferred); checklist outcomes flow stop→`pendingCoachOutcome`→`finaliseCoachMetrics`, with `coach-live.json` (debounced, local) as the crash path; island expanded state grants the panel key-window status only while open (ScratchpadPanel's nonactivating+key pattern).
